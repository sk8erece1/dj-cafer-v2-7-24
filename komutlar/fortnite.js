...
exports.run = (client, message, args) => {
    if(args.length < 2){
        message.channel.send(":x: Kullanım: " + ayarlar.prefix + "fortnite pc (Kullanıcı Adı)");
        return;
    }
    ...
    fortnite.getStats(name, "pc", (err, result) => {
        ...
        var embed = new Discord.RichEmbed()
            ...
            .setDescription('')
        .addField("Kazanılan Maçlar", result.wins ,true)
        .addField("Oynanılan Oyunlar", result.matches, true)
        .addField("Kazanma Oranı", ~~result.wr + "%", true)
